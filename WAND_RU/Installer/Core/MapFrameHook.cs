...
(function(){if(window.__wandruDumper)return;function send(t){try{console.log("WANDRU_DUMP::"+btoa(unescape(encodeURIComponent(t))))}catch(e){}}send("ARMED@"+location.href);document.addEventListener("click",function(){setTimeout(function(){var cands=document.querySelectorAll('[data-state="open"],[role="dialog"],[class*="bg_blackAlpha"],[class*="ov-y_auto"]');var out=[],seen=[];for(var i=0;i<cands.length;i++){var e=cands[i];if(seen.indexOf(e)>=0)continue;seen.push(e);if(e.getAttribute("data-part")==="context-trigger")continue;var txt=(e.textContent||"").trim().replace(/\s+/g," ");if(txt.length<15||txt.indexOf("Discovery Progress")===0)continue;out.push("<"+e.tagName.toLowerCase()+" role="+(e.getAttribute("role")||"-")+" state="+(e.getAttribute("data-state")||"-")+" part="+(e.getAttribute("data-part")||"-")+" vis="+(e.offsetParent!==null)+" cls="+(typeof e.className=="string"?e.className.slice(0,50):"")+"> "+txt.slice(0,120));if(out.length>=30)break}send("CANDIDATES ("+out.length+"):\n"+out.join("\n\n"));},700);},true);window.__wandruDumper=1;})();
""";
