...
(function(){if(window.__wandruDumper)return;function send(t){try{console.log("WANDRU_DUMP::"+btoa(unescape(encodeURIComponent(t))))}catch(e){}}send("ARMED@"+location.href);function depth(n){var d=0;while(n&&n.parentNode){d++;n=n.parentNode}return d}var recent=[];new MutationObserver(function(ms){ms.forEach(function(m){for(var i=0;i<m.addedNodes.length;i++){var n=m.addedNodes[i];if(n.nodeType===1&&n.textContent&&n.textContent.trim().length>30)recent.push(n);}});}).observe(document,{childList:true,subtree:true});document.addEventListener("click",function(){recent=[];setTimeout(function(){if(!recent.length)return;var best=recent[0],bd=depth(best);for(var i=1;i<recent.length;i++){var d=depth(recent[i]);if(d<bd||(d===bd&&recent[i].textContent.length>best.textContent.length)){best=recent[i];bd=d}}var bc=[],p=best;while(p&&p.tagName&&p!==document.body){bc.unshift(p.tagName.toLowerCase()+(typeof p.className=="string"&&p.className?"."+p.className.trim().replace(/\s+/g,"."):""));p=p.parentNode}send("BREADCRUMB: "+bc.slice(-7).join(" > "));send(best.outerHTML.slice(0,15000));},700);},true);window.__wandruDumper=1;})();
""";
