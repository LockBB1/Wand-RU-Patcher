...
(function(){if(window.__wandruDumper)return;function send(t){try{console.log("WANDRU_DUMP::"+btoa(unescape(encodeURIComponent(t))))}catch(e){}}send("ARMED@"+location.href);function depth(n){var d=0;while(n&&n.parentNode){d++;n=n.parentNode}return d}var recent=[];new MutationObserver(function(ms){ms.forEach(function(m){for(var i=0;i<m.addedNodes.length;i++){var n=m.addedNodes[i];if(n.nodeType===1&&n.textContent&&n.textContent.trim().length>30)recent.push(n);}});}).observe(document,{childList:true,subtree:true});document.addEventListener("click",function(){recent=[];setTimeout(function(){var panel=document.querySelector('div[class*="bg_black"][class*="ov-y_auto"]');if(!panel&&recent.length){var best=recent[0],bd=depth(best);for(var i=1;i<recent.length;i++){var d=depth(recent[i]);if(d<bd){best=recent[i];bd=d}}panel=best}if(!panel)return;send("PANEL state="+(panel.getAttribute("data-state")||"?")+" role="+(panel.getAttribute("role")||"?")+" cls="+(typeof panel.className=="string"?panel.className.slice(0,140):""));send(panel.outerHTML.slice(0,20000));},700);},true);window.__wandruDumper=1;})();
""";
