...
(function(){if(window.__wandruDumper)return;function send(t){try{console.log("WANDRU_DUMP::"+btoa(unescape(encodeURIComponent(t))))}catch(e){}}send("ARMED@"+location.href);document.addEventListener("click",function(){setTimeout(function(){var panel=document.querySelector('[data-part="content"][data-state="open"]')||document.querySelector('[role="dialog"][data-state="open"]:not([hidden])');if(!panel){send("NO-OPEN-DIALOG");return}var titId=panel.getAttribute("aria-labelledby"),descId=panel.getAttribute("aria-describedby");var tit=titId&&document.getElementById(titId),desc=descId&&document.getElementById(descId);send("PANEL state="+panel.getAttribute("data-state")+" titId="+titId+" descId="+descId);send("TITLE: "+(tit?tit.outerHTML.slice(0,900):"(none)"));send("DESC: "+(desc?desc.outerHTML.slice(0,9000):"(none)"));send("FULL: "+panel.outerHTML.slice(0,18000));},700);},true);window.__wandruDumper=1;})();
""";
