...
- **Русифицировать локально + интернет** - то же, плюс всё непокрытое словарём допереводится онлайн
  (сервис MyMemory, без ключа) и кэшируется. Переводит практически всё.
  > ⚠️ У бесплатного онлайн-перевода лимит ~1000 слов/день. При превышении часть имён останется
  > на английском (сработает офлайн-фолбэк). Кэш можно очистить в настройках, там же виден его размер.
...
Wand будет отправлять непереведённые имена читов в онлайн-переводчик (MyMemory). По умолчанию режим
**выключен** - тогда всё строго локально, без сетевых запросов.
...
- **Отправляет ли программа куда-то данные?**
  ...
  Wand будет слать имена читов в онлайн-переводчик MyMemory. Сам установщик данные не шлёт никогда.
...
- **Перевод читов** - в `app.asar` вшивается renderer-скрипт (`cheat-hook.js`): перехватывает ответы
  трейнера, переводит имена читов офлайн-словарём, а в онлайн-режиме допереводит остаток через MyMemory
  с локальным кэшем. Словарь и движок - JS, генерируются из одного источника. Приоритет перевода:
