...
## 🔜 В разработке: перевод имён читов

Готовится перевод названий читов трейнеров (`Unlimited Health` → `Бесконечное здоровье`,
`Damage Multiplier` → `Множитель урона`), которые приходят с серверов Wand. Это **офлайн-движок**
на словаре + морфологических шаблонах (род, падежи, склейка составных имён) — без онлайн-переводчиков.

Замер на реальных трейнерах **62 игр** (10 жанровых срезов): **~51%** имён переводятся с верной
грамматикой; в геймерских жанрах выше — гонки 85%, экшн 68%, космос 65%, выживание 59%, шутеры 55%.
Остаток — жанр-специфичный жаргон отдельных игр, пополняется по мере запросов пользователей.

> Функция ещё **не подключена** к установщику — идёт интеграция в патч. Пока установщик делает
> только русификацию интерфейса (см. выше).
